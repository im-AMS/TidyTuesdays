...
returns |> 
  group_by(stock_symbol) |> 
  filter(running_return == max(running_return)) |> 
  mutate(label = glue("+{round(running_return,0)}%")) |> 
  ungroup()
...
rect <-  tibble(data.frame(
        xmin = as.Date(c("2020-01-01")),
        xmax = as.Date(c("2022-01-01")),
        ymin = c(0),
        ymax = c(9000)
      ))
...
returns |>
  ...
  # Label max points
  geom_point(data = returns_max, aes(x = date, y = running_return))+
  geom_text_repel(
    data = returns_max,
    aes(
      x = date,
    ...
    seed = 43,
    family = base_fnt,
    size = legend_fnt_size - 6/.pt ,
    fontface = "bold",
    segment.size = 1/.pt ,
    force_pull = 0,
    nudge_y = if_else(returns_max$stock_symbol %in% c("AMZN","AAPL"), 500, 0),

    # Do not repel from top edge
    ylim = c(-Inf, Inf),
    # align all text to 2021
    xlim = c(NA, as.numeric(as.Date("2021-01-01"))),
  )+
...
glue("The line chart(below) depicts the returns on FAANG stocks if they were invested on **Jan 1,2010** and held until **Dec 29,2022**.<br> 
     ...
     <span style='color:#A2AAAD'>**Apple**</span> with {sd_stat_func('AAPL')}. (*Bin width = {binwidth}%*).
     <br><br>
    If just returns are evaluated, investing a big portion of one's portfolio in <span style='color:#e50914'>**Netflix**</span> may result in a greater total portfolio return.<br> However, in recent trends, <span style='color:#e50914'>**Netflix**</span> has had a big drop and has the greatest intraday volatility in **2022**. If **investing** is the aim, <br>lowering portfolio exposure to <span style='color:#e50914'>**Netflix**</span> and boosting exposure to lower volatile equities like <span style='color:#A2AAAD'>**Apple**</span> is likely to deliver steady returns, <br>according to the statistics. To the contrary if **trading** is the goal, trading in the more volatile stocks might increase the chance of <br>generating intraday profit.
     ")
...
# convert out.png 2023_06_dist_FR.png -gravity center +append 2023_06_FR.png &&
# rm out.png
